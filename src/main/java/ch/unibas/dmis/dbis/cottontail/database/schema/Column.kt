...
class Column<T : Any>(private val definition: ColumnDefinition<T>) {
    ...
    inner class Tx constructor(private val txid: UUID, override val mode: AccessorMode) : Transaction {
        ...
        init {
            ...
            /* Generate the HTreeMap for this Tx. */
        ...
        fun writeIfAbsent(tid: Long, value: T): Boolean {
            if (this.mode === AccessorMode.READONLY) throw IllegalStateException(String.format("Tx '%s' is read-only; it cannot be used to make any changes.", this.txid))
            if (this.status === TransactionStatus.CLOSED) throw IllegalStateException(String.format("Tx '%s' has been closed; it cannot be used to make any changes.", this.txid))
