...
object DeferredFetchRewriteRule : RewriteRule {
    ...
    private fun deferredFetchOnEntityScan(node: EntityScanLogicalOperatorNode, ctx: QueryContext): OperatorNode? {
        ...
        while (next != null && next.groupId == originalGroupId) {
            /** Case 1: We reach end of tree and the columns fetched in the encountered [EntityScanLogicalOperatorNode] are not required at all --> This should not happen, but hey. */
            val output = next.output ?: return node.output?.copyWithOutput(EntityScanLogicalOperatorNode(originalGroupId, node.entity, emptyArray()))

            /** Case 2: We encounter a node that requires specific but not all of the original columns --> Defer fetching and return */
            val required = originalColumns.filter { it in output.requires }.toTypedArray()
            if (required.isNotEmpty() && required.size < originalColumns.size) {
                ...
                */
                ...
                var p = next.output!!.copyWithInputs().base.first().output!!.copyWithOutput(EntityScanLogicalOperatorNode(originalGroupId, node.entity, required))
    ...
    private fun deferredFetchOnFetch(node: FetchLogicalOperatorNode, ctx: QueryContext): OperatorNode? {
        ...
        var next: OperatorNode.Logical? = node
        while (next != null && next.groupId == originalGroupId) {
            /** Case 1: We reach end of tree and the columns fetched in the encountered [FetchLogicalOperatorNode] are not required at all --> Drop them. */
            val output = next.output ?: return node.copyWithOutput(node.input.copyWithInputs())

            /** Case 2: We encounter a node that requires specific but not all of the original columns --> Defer fetching and return */
            val required = originalColumns.filter { it in output.requires }.toTypedArray()
            if (required.isNotEmpty() && required.size < originalColumns.size) {
                val defer = originalColumns.filter { it !in required }.toTypedArray()
                var p: OperatorNode.Logical = FetchLogicalOperatorNode(node.input.copyWithInputs(), node.entity, required)
                p = next.output!!.copyWithInputs().base.first().output!!.copyWithOutput(p)
                p = FetchLogicalOperatorNode(p, node.entity, defer)
                p = next.output?.output?.copyWithOutput(p) ?: p
                return p
            }
