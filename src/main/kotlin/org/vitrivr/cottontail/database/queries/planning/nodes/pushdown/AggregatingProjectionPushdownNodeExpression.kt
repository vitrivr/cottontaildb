 ...
 * @version 1.0
 */
class AggregatingProjectionPushdownNodeExpression(val type: ProjectionType, val entity: Entity, val column: ColumnDef<*>? = null, val alias: String? = null) : AbstractNodeExpression() {
    ...
    init {
        ...
        when (this.type) {
            ...
            ProjectionType.COUNT_DISTINCT -> if (this.column == null) {
                throw QueryException.QuerySyntaxException("Projection of type $type must specify a column.")
            }
            ProjectionType.MEAN,
            ProjectionType.SUM,
            ProjectionType.MIN,
            ProjectionType.MAX -> if (this.column == null) {
                throw QueryException.QuerySyntaxException("Projection of type $type must specify a column.")
    ...
    override val cost: Cost
        get() = when (type) {
            ProjectionType.COUNT -> Cost(Costs.DISK_ACCESS_READ, Costs.MEMORY_ACCESS_READ, 0.0f)
            ProjectionType.COUNT_DISTINCT -> Cost(Costs.DISK_ACCESS_READ, Costs.MEMORY_ACCESS_READ, (this.output * this.column!!.physicalSize).toFloat())
            ProjectionType.EXISTS -> Cost(Costs.DISK_ACCESS_READ, Costs.MEMORY_ACCESS_READ, 0.0f)
            ProjectionType.SUM -> Cost(this.output * Costs.DISK_ACCESS_READ, this.output * Costs.MEMORY_ACCESS_READ, 0.0f)
            ProjectionType.MAX -> Cost(this.output * Costs.DISK_ACCESS_READ, this.output * Costs.MEMORY_ACCESS_READ, 0.0f)
            ProjectionType.MIN -> Cost(this.output * Costs.DISK_ACCESS_READ, this.output * Costs.MEMORY_ACCESS_READ, 0.0f)
            ProjectionType.MEAN -> Cost(this.output * Costs.DISK_ACCESS_READ, this.output * Costs.MEMORY_ACCESS_READ, 0.0f)
            else -> Cost.INVALID
        }
    ...
    override fun toStage(context: QueryPlannerContext): ExecutionStage {
        ...
        when (this.type) {
            ProjectionType.COUNT -> stage.addTask(EntityCountProjectionTask(this.entity))
            ProjectionType.EXISTS -> stage.addTask(EntityExistsProjectionTask(this.entity))
            ProjectionType.SUM -> stage.addTask(EntitySumProjectionTask(this.entity, this.column!!, this.alias))
            ProjectionType.MAX -> stage.addTask(EntityMaxProjectionTask(this.entity, this.column!!, this.alias))
            ProjectionType.MIN -> stage.addTask(EntityMinProjectionTask(this.entity, this.column!!, this.alias))
            ProjectionType.MEAN -> stage.addTask(EntityMeanProjectionTask(this.entity, this.column!!, this.alias))
            else -> {
            }
        }
        return stage
