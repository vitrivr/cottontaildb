                ...
                if (sort is SortPhysicalOperatorNode) {
                    ...
                    if (limit is LimitPhysicalOperatorNode) {
                        ...
                        val candidate = if (hints != null) {
                            scan.entity.listIndexes().find { it.canProcess(node.predicate) && hints.satisfies(it) }
                        } else {
                            scan.entity.listIndexes().find { it.canProcess(node.predicate) }
                        }
                        ...
                        if (candidate != null) {
                            when {
                                ...
                                candidate.produces.contains(column) -> {
                                    ...
                                    if (delta.isNotEmpty()) {
                                        p = FetchPhysicalOperatorNode(p, scan.entity, delta.toTypedArray())
                                    }
                                ...
                                candidate.produces.contains(node.predicate.column) -> {
                                    val index = IndexScanPhysicalOperatorNode(node.groupId, ctx.txn.getTx(candidate) as IndexTx, node.predicate)
