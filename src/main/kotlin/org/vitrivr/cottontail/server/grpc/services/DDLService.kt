...
class DDLService(val catalogue: Catalogue, override val manager: TransactionManager) : DDLGrpc.DDLImplBase(), TransactionService {
     ...
     */
    override fun listSchemas(request: CottontailGrpc.ListSchemaMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        ...
        try {
            ...
            val message = "Failed to fetch list of schemas because because transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
     ...
     */
    override fun createSchema(request: CottontailGrpc.CreateSchemaMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        /* Obtain transaction or create new one. */
        ...
        try {
            ...
            val message = "Failed to create schema '${request.schema.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
     ...
     */
    override fun dropSchema(request: CottontailGrpc.DropSchemaMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        /* Obtain transaction or create new one. */
        val schemaName = request.schema.fqn()
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to drop schema '${request.schema.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
     ...
     */
    override fun entityDetails(request: CottontailGrpc.EntityDetailsMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        /* Obtain transaction or create new one. */
        ...
        try {
            ...
            val message = "Failed to fetch entity information for '${request.entity.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun createEntity(request: CottontailGrpc.CreateEntityMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        /* Obtain transaction or create new one. */
        ...
        try {
            ...
            val message = "Failed to create entity '${request.definition.entity.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun dropEntity(request: CottontailGrpc.DropEntityMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to ddrop entity '${request.entity.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun truncateEntity(request: CottontailGrpc.TruncateEntityMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to truncate entity '${request.entity.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun optimizeEntity(request: CottontailGrpc.OptimizeEntityMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to optimize entity '${request.entity.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun listEntities(request: CottontailGrpc.ListEntityMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        /* Extract schema name. */
        ...
        try {
            ...
            val message = "Failed to list entities for schema '${request.schema.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun createIndex(request: CottontailGrpc.CreateIndexMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to create index '${request.definition.index.name.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun dropIndex(request: CottontailGrpc.DropIndexMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to drop index '${request.index.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
     ...
     */
    override fun rebuildIndex(request: CottontailGrpc.RebuildIndexMessage, responseObserver: StreamObserver<CottontailGrpc.QueryResponseMessage>) = this.withTransactionContext(request.txId) function@{ tx ->
        val queryId = UUID.randomUUID().toString()
        ...
        try {
            ...
            val message = "Failed to rebuild index '${request.index.fqn()}': Transaction ${e.txId} could not be resumed."
            LOGGER.info(message)
            responseObserver.onError(Status.FAILED_PRECONDITION.withDescription(message).asException())
        } catch (e: DatabaseException.SchemaDoesNotExistException) {
