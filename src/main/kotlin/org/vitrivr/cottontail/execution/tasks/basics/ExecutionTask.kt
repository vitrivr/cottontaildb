...
import org.vitrivr.cottontail.execution.tasks.TaskExecutionException
...
abstract class ExecutionTask(name: String) : Task<String, Recordset>() {
     ...
     */
    protected fun first(): Recordset? = if (this.parentResults?.first!!.isSuccess) {
        this.parentResults?.first!!.result
    } else {
        null
    }
