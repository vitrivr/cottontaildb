...
//                            volume.clear(oldOffset,oldOffset+sizeUp)
    ...
    override fun preallocate(): Long {
        assertNotClosed()
        val recid = CottontailUtils.lock(structuralLock){
            allocateRecid()
        }
        CottontailUtils.lockWrite(locks[recidToSegment(recid)]) {
            //            if (CC.ASSERT) {
...
//            volume.putData(chunkOffset+8, output, remSize.toInt(), chunkSize.toInt())
        ...
        CottontailUtils.lockWrite(locks[segment]) {
            if (di != null) {
                if(di.pos==0) {
                    ...
                    //allocate space
                    val volOffset = CottontailUtils.lock(structuralLock) {
    ...
    /**
     * Creates and returns a [LongIterator] for all used record IDs contained in this [CottontailStoreWAL].
     *
     * @return [LongIterator]
     */
    @Deprecated("This function is deprecated. Use RecordIdIterator() instead.")
...
//            volume.clear(offset,offset+currentSize) //TODO incremental clear
    ...
    inner class RecordIdIterator(range: LongRange = 2L..this@CottontailStoreWAL.maxRecid) : LongIterator() {
         ...
         * end of the search space (maximumRecordId) without finding a valid entry. In this case, the method will return EOF_ENTRY
         *
