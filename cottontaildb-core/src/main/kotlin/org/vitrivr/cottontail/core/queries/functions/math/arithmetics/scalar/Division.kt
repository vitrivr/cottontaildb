...
/**
 * A [Function] to divide two scalar values.
 *
 * @author Ralph Gasser
 * @version 1.0.0
 */
sealed class Division<T: Value>(val type: Types<T>): Function<T> {
    companion object: FunctionGenerator<Value> {
        private val FUNCTION_NAME = Name.FunctionName.create("div")

        override val signature: Signature.Open
            get() = Signature.Open(FUNCTION_NAME, arrayOf(Argument.Numeric, Argument.Numeric))

        override fun obtain(signature: Signature.SemiClosed): Division<*> {
    ...
    /**
     * Division of two [IntValue].
     */
    class Int: Division<IntValue>(Types.Int) {
    ...
    /**
     * Division of two [LongValue].
     */
    class Long: Division<LongValue>(Types.Long)  {
    ...
    /**
     * Division of two [FloatValue].
     */
    class Float: Division<FloatValue>(Types.Float) {
    ...
    /**
     * Division of two [IntValue]s.
     */
    class Double: Division<DoubleValue>(Types.Double) {
