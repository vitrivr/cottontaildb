 ...
 * @version 1.0.0
 */
class BooleanVectorByteBufferTablet(override val size: Int, logicalSize: Int, direct: Boolean): Tablet.BooleanVector {
    init {
        require(this.size % Int.SIZE_BITS == 0) { "Tablet size must be a multiple of 32." }
    }

    /** The [Types] of a [BooleanByteBufferTablet] is always [Types.Boolean]. */
    override val type: Types<BooleanVectorValue> = Types.BooleanVector(logicalSize)

    /** The raw [ByteBuffer] backing this [BooleanByteBufferTablet]. */
