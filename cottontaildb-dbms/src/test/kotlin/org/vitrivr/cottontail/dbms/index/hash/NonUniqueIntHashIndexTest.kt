...
class NonUniqueIntHashIndexTest : AbstractIndexTest() {
    ...
    fun testFilterEqualNegative() {
        ...
        var count = 0
        val predicate = BooleanPredicate.Comparison(ComparisonOperator.Binary.Equal(ctx.bindings.bind(this.columns[0]), ctx.bindings.bind(IntValue(this.random.nextInt(100, Int.MAX_VALUE)))))
        indexTx.filter(predicate).use {
            it.forEach { count += 1 }
        }
