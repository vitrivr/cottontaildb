 ...
 * @version 1.0.0
 */
class LpBounds(query: RealVectorValue<*>, marks: VAFMarks, val p: Int) : Bounds {
        ...
        for (j in signature.cells.indices) {
            when {
                ri[j] < this.rq[j] -> {
                    ...
                    this.ub += this.lat[j][ri[j].toInt()][0]
                }
                ri[j] == this.rq[j] -> {
                    this.ub += max(this.lat[j][ri[j].toInt()][0], this.lat[j][ri[j] + 1][1])
                }
                ri[j] > this.rq[j] -> {
                    this.lb += this.lat[j][ri[j].toInt()][1]
