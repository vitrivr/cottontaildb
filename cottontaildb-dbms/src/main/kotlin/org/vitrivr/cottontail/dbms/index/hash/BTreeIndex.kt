 ...
 * @version 4.0.0
 */
class BTreeIndex(name: Name.IndexName, parent: DefaultEntity) : AbstractBTreeIndex(name, parent) {
     ...
     */
    inner class Tx(parent: DefaultEntity.Tx) : AbstractBTreeIndex.Tx(parent) {
        /** The Xodus [Store] used to store entries in the [BTreeIndex]. */
        override var store: Store = this.xodusTx.environment.openStore(this@BTreeIndex.name.storeName(), StoreConfig.WITH_DUPLICATES_WITH_PREFIXING, this.xodusTx)

        /**
         * Truncates (i.e., clears) the [Index] backed by this [IndexTx].
         */
        override fun truncate() = this.txLatch.withLock {
            this.xodusTx.environment.truncateStore(this@BTreeIndex.name.storeName(), this.xodusTx)
            this.store = this.xodusTx.environment.openStore(this@BTreeIndex.name.storeName(), StoreConfig.WITH_DUPLICATES_WITH_PREFIXING, this.xodusTx)
        }
    }
}
