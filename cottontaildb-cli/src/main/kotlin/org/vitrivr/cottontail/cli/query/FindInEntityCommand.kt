...
class FindInEntityCommand(client: SimpleClient): AbstractCottontailCommand.Query(client, name = "find", help = "Find within an entity by column-value specification") {
    ...
    private val pattern: Boolean by option("-p", "--pattern", help="Flag indicating that the query value should be interpreted as pattern.").flag("--no-pattern", default = false)
